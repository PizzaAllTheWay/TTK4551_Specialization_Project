...
\\ \\
To capture environmental and geometric effects, the deterministic model in Equation \eqref{eq:range-bearing-model-deterministic} is extended with both stochastic noise and an angle and range dependent disturbance term. The complete measurement model is formulated as
\begin{equation}
    h_{\text{noisy}}(\mathbf{x}, m^i) = h(\mathbf{x}, m^i) + \Delta h(\mathbf{x}, m^i, \phi_i) + \mathbf{v}^i(r_i)
    \label{eq:range-bearing-model-extended}
\end{equation}
where $h(\mathbf{x}, m^i)$ is the deterministic range-bearing model defined in \eqref{eq:range-bearing-model-deterministic}, $\Delta h(\mathbf{x}, m^i, \phi_i)$ represents systematic disturbances caused by angle dependent reflection and refraction, and $\mathbf{v}^i(r_i)$ is a zero mean Gaussian noise term with range dependent covariance.  
\\ \\
The local incidence angle $\phi_i$ is defined as the angle between the sonar beam direction and the line of sight to the landmark in the body frame, computed as
$$
    \phi_i = \mathrm{atan2}(m_b^i{}_y,\, m_b^i{}_x),
    \qquad
    m_b^i = R_n^b(\mathbf{q}) \left( m^i - \mathbf{p}_{b/O}^{n} \right)
$$
where $R_n^b(\mathbf{q})$ transforms the landmark position from the navigation frame to the body frame. The angle $\phi_i$ varies with the ASV yaw orientation and thus changes as the same landmark is viewed from different headings, directly influencing reflection strength and measurement geometry.  
\\ \\
The angle dependent disturbance term $\Delta h(\mathbf{x}, m^i, \phi_i)$ models systematic deviations due to incidence effects and is approximated as
$$
    \Delta h(\mathbf{x}, m^i, \phi_i) \approx
    \begin{bmatrix}
        \kappa_r (1 - \cos{\phi_i}) \\
        \kappa_\theta \sin{\phi_i}
    \end{bmatrix}
$$
where $\kappa_r$ and $\kappa_\theta$ are empirically tuned parameters describing the sensitivity of range and bearing to the incidence angle. The range component decreases for grazing angles due to weaker reflections and shadowing, while the bearing component introduces a small angular bias from asymmetric scattering.  
\\ \\
The stochastic noise term $\mathbf{v}^i(r_i) = [v_r^i, v_\theta^i]^\top$ represents random measurement uncertainty that approximately increases with the distance to the observed landmark. The landmark position in the body frame and the corresponding predicted range are obtained as
$$
    r_i = \sqrt{(m_b^i{}_x)^2 + (m_b^i{}_y)^2}, 
    \qquad 
    m_b^i = R_n^b(\mathbf{q}) \left( m^i - \mathbf{p}_{b/O}^{n} \right)
$$
and the range dependent measurement covariance is approximated by
$$
    \mathbf{R}_z(r_i) \approx
    \begin{bmatrix}
        \sigma_r^2(1 + \alpha_r r_i^2) & 0 \\
        0 & \sigma_\theta^2(1 + \alpha_\theta r_i^2)
    \end{bmatrix}
$$
where $\sigma_r$ and $\sigma_\theta$ represent the nominal standard deviations of the range and bearing measurements at close distance, while $\alpha_r$ and $\alpha_\theta$ are empirical scaling factors that approximate how these uncertainties grow quadratically with increasing range due to acoustic attenuation, multipath effects, and beam spreading.
