...
After swath processing, the sonar data is geometrically corrected, radiometrically normalized, and expressed in ground coordinates. The next step is to reconstruct a local seafloor map that spatially represents the acoustic backscatter intensity in a consistent coordinate frame. This process, referred to as \textit{``probabilistic mapping''}, forms the foundation for subsequent feature extraction and SLAM data association.  
\\ \\
Traditional Cartesian mapping methods assign a single intensity value to each grid cell based on nearby sonar samples, often using distance based interpolation such as k-Nearest Neighbor (kNN). While fast and efficient, this assumes that each sonar measurement originates from a precise point on the seafloor, ignoring beam width, acoustic footprint, and uncertainty in the range angle domain. As a result, the generated map can appear sharp but physically inaccurate and unstable under noisy or sparse sampling conditions.  
\\ \\
In contrast, the probabilistic mapping approach distributes the contribution of each sonar return across neighboring grid cells according to a spatial likelihood function that models the sonar beam geometry and measurement uncertainty. Each beam is treated as a probabilistic field rather than a deterministic ray, capturing the spread of possible reflection locations. This results in a continuous, uncertainty aware reflectivity map that better represents the true physical response of the seafloor.  
\\ \\
The goal is not only to visualize the seabed but to construct a stable and repeatable spatial representation suitable for landmark detection. The probabilistic formulation improves the consistency of features across consecutive maps and reduces sensitivity to noise, making it a more robust foundation for SLAM compared to purely interpolated Cartesian intensity grids.
...
\paragraph{Conlcusion} \mbox{}\\[0.5em] \noindent 
...
The remaining grid cells are then pruned by angular visibility using the observation angle $\theta_{obs}(q)$ (see Equation \ref{eq:local-map-generation-observation-angle}). The precomputed distances $r_q$ from the previous stage are reused to calculate $\theta_{obs}(q)$ efficiently. Cells outside the effective horizontal beam aperture are removed:
$$
    |\theta_{obs}(q)| < \frac{\alpha_h}{2}
$$
...
For the remaining candidates, the illumination probability $P_m(q)$ is evaluated using the beam model in Equation \ref{eq:local-map-generation-probability-weight-individual-swaths}. Cells with negligible contribution are discarded using a small predefined threshold $\epsilon$, yielding the final refined set:
$$
    Q_m = \left\{
        q \in Q \; \bigg| \;
