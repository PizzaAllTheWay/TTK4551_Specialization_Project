...
Once the local map $M$ has been generated, the next step is to identify distinct and stable seafloor features that can serve as landmarks for SLAM. These landmarks correspond to repeatable acoustic structures, typically bright echoes or dark shadow regions that remain consistent across consecutive local maps and provide reliable reference points for localization and data association.  
\\ \\
The detection process operates directly on the local reflectivity map $M$ without requiring any global reference.  
The map is first filtered using a edge preservation filter to reduce speckle noise while preserving strong spatial gradients.
\\ \\
From the resulting field, locally dominant regions are extracted based on intensity contrast and spatial coherence. Compact high intensity areas are identified as \textit{``echo landmarks''}, representing reflective or elevated objects, while elongated low intensity regions correspond to \textit{``shadow landmarks''} created by acoustic occlusion or terrain depressions.  
\\ \\
Each detected landmark is characterized by its centroid position $\mathbf{p}_L = [x_L, y_L]^\top$ in the local Cartesian frame and is assigned a compact descriptor summarizing the surrounding texture and reflectivity pattern. These descriptors are later used for efficient nearest neighbor matching across consecutive map updates in the SLAM data association stage.  
\\ \\
For integration with the SLAM formulation, every landmark is expressed as a local measurement in range and bearing relative to the vehicle, which is assumed to be positioned at the origin $(0,0)$ of the local map with its heading aligned with the $x$-axis:
$$
    \mathbf{z}^i = [z_r^i, \, z_\theta^i]^\top
$$
This formulation provides a direct and self contained measurement model consistent with the range bearing model $h(\mathbf{x}, \mathbf{m}^i)$ used in SLAM (see Equation \ref{eq:range-bearing-measurement-model}).  
\\ \\
Each extracted landmark is stored together with its descriptor, centroid position, and confidence score, forming the interface between the local mapping stage and the SLAM data association process, which determines whether a given landmark corresponds to an already known feature or represents a newly observed one.  
\begin{figure}[H]
    \centering
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{Pictures/Local_Map_Generation/Feature_Extraction/kernal_operation.png}
...
After each landmarks centroid $\mathbf{p}_L = [x_L,\, y_L]^\top$ is extracted (see Equation \ref{eq:local-map-generation-centroid}), it must be expressed in a measurement form compatible with the SLAM observation model $h(\mathbf{x}, \mathbf{m}^i)$ (see Equation \ref{eq:range-bearing-model-deterministic}).  
\\ \\
In our setup, both the vehicle and the detected landmarks are defined within the same local Cartesian frame. The vehicle is assumed to be located at the origin $(0,0)$ with its heading aligned along the $x$-axis. Under this assumption, the transformation from Cartesian to range bearing coordinates becomes direct and self contained:
\begin{equation}
    \mathbf{z}^i =
    \begin{bmatrix}
        z_r^i \\[0.3em]
...
\end{equation}
Here, $z_r^i$ denotes the measured range to the $i$-th landmark and $z_\theta^i$ represents its bearing angle relative to the vehicles forward axis.  
\\ \\
This polar representation is essential for SLAM because the measurement model $h(\mathbf{x}, \mathbf{m}^i)$ typically operates in range bearing form, making the observation directly comparable to predicted sensor readings during the update stage. Expressing the centroid in this format eliminates the need for any further coordinate transformations,  
allowing straightforward integration with the sensor model and uncertainty propagation.  
...
\\ \\
To properly integrate these measurements into the SLAM optimization, each observation is assigned a covariance matrix $\mathbf{R}_{z_i}$ that quantifies the expected error in range and bearing. This matrix defines the confidence associated with the measurement constraint in the graph, weighting its influence during the nonlinear least squares optimization.  
\\ \\
...
\\ \\
Within the graph based SLAM framework, this covariance directly defines the information matrix $\mathbf{\Omega}_{z_i} = \mathbf{R}_{z_i}^{-1}$ used to weight the measurement residuals during optimization, ensuring that reliable, close landmarks contribute more strongly than distant or uncertain ones.
...
\begin{equation}
    \mathbf{d}_L^i = [\, \mathbf{d}_L^{\text{strong}},\, \mathbf{d}_L^{\text{weak}} \,]
...
$$
    \mathcal{L}_i = \{\mathbf{z}^i,\, \mathbf{R}_{z^i},\, \mathbf{d}_L^i\}
$$
